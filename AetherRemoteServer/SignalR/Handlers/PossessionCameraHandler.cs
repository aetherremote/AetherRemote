    ...
    ILogger<PossessionMovementHandler> logger)
{
    ...
    public async Task Handle(string senderFriendCode, PossessionCameraRequest request, IHubCallerClients clients)
    {
        ...
        var command = new PossessionCameraCommand(senderFriendCode, request.Zoom, request.X, request.Y, request.Z);
