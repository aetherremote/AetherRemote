...
public partial class HonorificView
{
    public void Draw()
    {
        ...
        SharedUserInterfaces.ContentBox("TitleSearch", AetherRemoteColors.PanelColor, true, () =>
        {
            ...
            ImGui.InputTextWithHint("##TitleSearchBar", "Search", ref _searchTerm, 32);
        ...
        if (ImGui.BeginChild("##HonorificContextBoxDisplay", honorificContextBoxSize, true, ImGuiWindowFlags.NoScrollbar))
        {
            ...
            foreach (var (character, titles) in FilteredTitles)
            {
        ...
        SharedUserInterfaces.ContentBox("HonorificSend", AetherRemoteColors.PanelColor, false, () =>
        {
            ...
            else if (_selectedTitle is null)
            {
                ImGui.BeginDisabled();
            ...
            else
            {
                ...
                else
                {
                    if (ImGui.Button("Send Honorific", size))
                        _ = SendHonorific().ConfigureAwait(false);
                }
            }
        });
    ...
    private void DrawTitleOption(SeStringDrawParams parameters, HonorificCustomTitle honorific)
    {
        ...
        if (ImGui.Selectable($"##{honorific.Title}", _selectedTitle == honorific))
            _selectedTitle = honorific;
