...
public partial class MoodlesView
{
    ...
    private void DrawContent()
    {
        ...
        SharedUserInterfaces.ContentBox("MoodlesFromMoodles", AetherRemoteColors.PanelColor, true, () =>
        {
            ...
            ImGui.InputTextWithHint("##MoodlesSearchBar", "Search", ref _searchTerm, 32);
        ...
        if (ImGui.BeginChild("##MoodlesContextBoxDisplay", moodlesContextBoxSize, true, ImGuiWindowFlags.NoScrollbar))
        {
            ...
            for (var i = 0; i < FilteredMoodles.Count; i++)
            {
                DrawDisplayMoodleButton(FilteredMoodles[i], i, padding, font, fontSize);
    ...
    private void DrawDisplayMoodleButton(Moodle moodle, int index, Vector2 padding, ImFontPtr font, float fontSize)
    {
        ...
        else
        {
            if (ImGui.Button($"##{moodle.Info.Guid}", DefaultMoodleButtonSize))
                _selectedMoodleIndex = index;
        }
        
        if (TryGetIcon(moodle.Info.IconId) is { } icon)
        {
