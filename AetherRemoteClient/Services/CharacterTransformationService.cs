    ...
    PenumbraDependency penumbraDependency)
{
    ...
    {
        // Convert to JObject
        if (GlamourerDependency.ConvertGlamourerBase64StringToJObject(glamourerCode) is not { } glamourerCodeAsComponents)
    ...
    public async Task<ApplyGenericTransformationResult> ApplyGenericTransformation(JObject glamourerJObject, GlamourerApplyFlags flags)
    {
        ...
        if (await glamourerDependency.GetDesignComponentsAsync().ConfigureAwait(false) is not { } local)
        ...
        if (GlamourerDependency.CreateJObjectToRevertExistingAdvancedDyes(local, glamourerJObject) is not { } glamourerCodeAsComponentsWithoutAdvancedDyes)
        ...
        return await glamourerDependency.ApplyDesignAsync(glamourerCodeAsComponentsWithoutAdvancedDyes, flags).ConfigureAwait(false) 
    ...
    public async Task ApplyPerm(PermanentTransformationData permanentTransformationData)
    {
        ...
        // Get local character data
        if (await glamourerDependency.GetDesignComponentsAsync().ConfigureAwait(false) is not { } localDesignJObject)
            return;
        ...
        // Apply Glamourer
        await glamourerDependency.ApplyDesignAsync(convertedDesign, permanentTransformationData.GlamourerApplyType);

        // Apply Mods
        if (permanentTransformationData.ModMetaData is not null && permanentTransformationData.ModPathData is not null)
            await penumbraDependency.AddTemporaryMod(collection, permanentTransformationData.ModPathData, permanentTransformationData.ModMetaData);

        // Apply Customize
        if (permanentTransformationData.CustomizePlusData is not null)
            await customizePlusDependency.ApplyCustomize(permanentTransformationData.CustomizePlusData);
        ...
        if (permanentTransformationData.MoodlesData is not null)
            if (await Plugin.RunOnFramework(() => Plugin.ObjectTable[0]?.Address).ConfigureAwait(false) is { } address)
                await moodlesDependency.SetMoodles(address, permanentTransformationData.MoodlesData);
    }

    private async Task<Guid?> TryRemoveExistingMods()
    {
        // Get Current Collection
        var collection = await penumbraDependency.GetCollection().ConfigureAwait(false);
        ...
        if (await penumbraDependency.CallRemoveTemporaryMod(collection).ConfigureAwait(false) is false)
            return null;
    ...
    private async Task<List<ICharacterAttribute>?> TryGetPlayerAttributes(CharacterAttributes characterAttributes, IGameObject gameObject, Guid collection)
    {
        ...
        var glamourerAttribute = new GlamourerAttribute(this, glamourerDependency, gameObject.ObjectIndex);
        ...
        if ((characterAttributes & CharacterAttributes.Mods) is CharacterAttributes.Mods)
        {
            ...
            var modsAttribute = new ModsAttribute(penumbraDependency, collection, gameObject.ObjectIndex);
        ...
        if ((characterAttributes & CharacterAttributes.Moodles) is CharacterAttributes.Moodles)
        {
            ...
            var moodlesAttribute = new MoodlesAttribute(moodlesDependency, gameObject.Address);
        ...
        if ((characterAttributes & CharacterAttributes.CustomizePlus) is CharacterAttributes.CustomizePlus)
        {
            ...
            var customizePlusAttribute = new CustomizePlusAttribute(customizePlusDependency, gameObject.Name.TextValue);
